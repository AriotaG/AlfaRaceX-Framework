...
- `PARITY / TARGET`: portable logic is complete; MCU/HAL binding is still required.
...
| Dynamic Shift extension | AUDIT | ARX extension; IPC-only Dynamic gating still to isolate |
...
| Runtime config | IMPROVED | version + CRC; two-slot target storage still to bind |
